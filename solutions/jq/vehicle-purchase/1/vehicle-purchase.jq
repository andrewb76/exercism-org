...
def choose_vehicle: if .[0] < .[1] then .[0] else .[1] end | "\(.) is clearly the better choice." | debug;
...
def resell_price: .original_price as $p | if .age < 3 then 0.8 elif .age > 10 then 0.5 else 0.7 end | . * $p;
